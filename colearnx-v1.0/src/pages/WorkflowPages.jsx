...
function Records({ rows, columns }) {
  ...
  return <div className="responsive-table"><table><thead><tr>{columns.map(([key, title]) => <th key={key}>{title}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={row.id || index}>{columns.map(([key]) => <td key={key}>{row[key] === null ? "Not tracked" : typeof row[key] === "object" ? "See associated record" : String(row[key] ?? "Not supplied")}</td>)}</tr>)}</tbody></table></div>;
}
...
function PublishingAnalytics({ kind }) {
  ...
  return <Card><h3>Sales and usage</h3><DateRange value={range} onChange={setRange} /><RequestState state={state} reload={reload}>
    <dl className="receipt-details">{[["sales", "Sales"], ["revenuePoints", "Revenue in points"], ["downloads", "Downloads"], ["views", "Views"]].map(([key, label]) => <div key={key}><dt>{label}</dt><dd>{key === "views" && state.data?.views === null ? "Not tracked" : state.data?.[key] ?? "Not supplied"}</dd></div>)}</dl>
...
export function AdminOperationsPage() {
  ...
  const confirm = async () => {
    setBusy(true); setError("");
    try { if (pending.type === "points") await api.adjustPoints(pending.input, pending.requestKey); else await api.decideReport(pending.id, { decision, reason: reason.trim() }, pending.requestKey); setPending(null); setSuccess("The service accepted and recorded the administration action."); void reload(); }
    catch (error) { setError(api.serviceMessage(error)); } finally { setBusy(false); }
  };
  ...
  return <div className="stack"><Card><h2>Administration operations</h2><div className="button-row" role="group" aria-label="Administration views">{[["reports", "Reports"], ["points", "Points adjustment"], ["audit", "Audit log"], ["activity", "Activity report"]].map(([key, label]) => <Button key={key} variant={tab === key ? "primary" : "secondary"} aria-pressed={tab === key} onClick={() => { setTab(key); setSuccess(""); }}>{label}</Button>)}</div></Card>
    ...
    {pending && <Modal title={pending.type === "points" ? "Confirm wallet adjustment" : "Review reported item"} onClose={() => !busy && setPending(null)} footer={<><Button variant="secondary" disabled={busy} onClick={() => setPending(null)}>Cancel</Button><Button disabled={busy || (pending.type === "report" && reason.trim().length < 5)} onClick={() => void confirm()}>{busy ? "Submitting…" : "Confirm administration action"}</Button></>}>
      {pending.type === "points" ? <><p>Account: {pending.input.userId}</p><p>Available: {pending.input.deltaPoints > 0 ? "+" : ""}{pending.input.deltaPoints} points</p><p>{pending.input.reason}</p><p>This changes a financial balance and cannot be undone by closing this dialog.</p></> : <><FormField label="Decision"><select value={decision} onChange={(e) => setDecision(e.target.value)}><option value="resolved">Resolved</option><option value="dismissed">Dismissed</option></select></FormField><FormField label="Decision reason"><textarea value={reason} maxLength={4000} onChange={(e) => setReason(e.target.value)} /></FormField></>}
