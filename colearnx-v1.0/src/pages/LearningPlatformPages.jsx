    ...
    <div id={panelId} hidden={!expanded} className="delivery-panel">
      ...
      {delivery && <>
        {modes.includes("cloud") && <section><b>Cloud course files</b>
          {(delivery.assets || []).filter((asset) => !isVideoAsset(asset) && (!asset.status || asset.status === "ready")).map((asset) =>
          ...
          {!(delivery.assets || []).some(asset => !isVideoAsset(asset) && (!asset.status || asset.status === "ready")) && <p>No authorised attachment is available.</p>}
        </section>}
...
export function PurchasesPage() {
  ...
  return <div className="stack">{items.map((item) => <Card key={item.id} className="learning-row"><span className="list-icon">{item.kind === "course" ? <GraduationCap size={18} /> : <FileArchive size={18} />}</span><div><b>{item.title}</b><small>{item.kind === "course" ? deliveryLabel(item.deliveryModes) : "Digital content"} · {item.price} points</small><p className="learning-policy">{item.kind === "content" ? "Request a short-lived, purchase-authorised download link." : deliveryDisclosures(item).join(" ")}</p></div>{item.kind === "content" ? <ContentDownloadButton contentVersionId={item.productId} /> : <CourseDeliveryPanel item={item} />}<RefundLink item={item} /><Link className="button secondary sm" to={`/checkout-success/${item.order.id}`}>View receipt</Link></Card>)}</div>;
}

export function RefundPage() {
  const { id } = useParams(); const [search] = useSearchParams(); const navigate = useNavigate();
  const { orders, submitRefund } = usePlatform(); const course = refundPurchase(orders, id, search.get("orderItem"));
  const [reason, setReason] = useState(""), [busy, setBusy] = useState(false), [error, setError] = useState("");
  const request = useRef(null);
  const [evidence, setEvidence] = useState(null), [evidenceError, setEvidenceError] = useState("");
