...
let accessToken = typeof window === "undefined" ? "" : window.sessionStorage.getItem(accessTokenKey) || "";
let csrfToken = typeof window === "undefined" ? "" : window.sessionStorage.getItem(csrfTokenKey) || "";
...
export function setAccessToken(token) { accessToken = token || ""; if (typeof window === "undefined") return; if (accessToken) window.sessionStorage.setItem(accessTokenKey, accessToken); else window.sessionStorage.removeItem(accessTokenKey); }
export const hasAccessToken = () => Boolean(accessToken);
export function setCsrfToken(token) { csrfToken = token || ""; if (typeof window === "undefined") return; if (csrfToken) window.sessionStorage.setItem(csrfTokenKey, csrfToken); else window.sessionStorage.removeItem(csrfTokenKey); }
