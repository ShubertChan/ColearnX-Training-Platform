...
| Course authoring | `POST/PATCH /courses` accepts buyer-only `fulfilmentInstructions`, `trainerContact`, `joinUrl` and `progressTrackingType`; hosted-video duration is worker-owned and is never supplied by the browser | Trainer ownership plus approved certification capability |
