...
The React client uses this API directly: access tokens and CSRF tokens are kept in browser session storage, while the refresh token is an HTTP-only cookie. On a new tab or an expired access token, the client gets `/auth/csrf` and then calls `/auth/refresh` with `X-CSRF-Token`. The Vite development server proxies `/api` to port 3001. Marketplace listings, orders, refunds, role applications, creator submissions and administrator queues are API-backed. The browser-persisted mixed cart is only a recoverable draft; the server must revalidate prices, policies, roles, availability and entitlements at checkout. Course delivery, video-progress, public-profile, privacy-request and password-reset contracts required by this frontend are documented in [`FRONTEND_BACKEND_HANDOFF.md`](./FRONTEND_BACKEND_HANDOFF.md) and must be implemented by the separately deployed API.
