...
锁定、账号是否存在、剩余时间——这类信息**只能发到注册邮箱**，
绝不能出现在登录页面上。

判断标准不是"内容敏不敏感"，而是**收信人是否已被证明拥有这个账号**。
邮箱、短信、已登录设备是已验证渠道，可以承载账号状态；
登录页是任何人都能看到的公开渠道，只能承载对所有人一致的内容。

前端那条「多次失败」提示之所以安全，是因为它由**浏览器自己的失败计数**驱动，
不是服务端告诉它的——对不存在的邮箱失败 5 次，会出现完全一样的提示。
改动那段逻辑时务必保持这个性质。

W4 做 2FA 时会再用到同一个判断：恢复码发到哪、step-up 验证码走什么通道。

### 约束一之二：MFA 的两个密钥不能合并

`MFA_SECRET_KEY` 加密 TOTP 密钥，`MFA_CHALLENGE_SECRET` 签发挑战令牌。
env.ts 在启动时强制两者不同。

合并看起来省事，实际会造成：轮换加密密钥会静默作废所有在途的挑战令牌，
反之亦然——这种耦合没人会发现，直到某次轮换出事。

`MFA_SECRET_KEY` **丢失即不可恢复**：所有已绑定的验证器全部失效。
备份它的谨慎程度应等同于数据库本身。
...
`PlatformContext` 里的 `establishSession` 是单因子和双因子登录**共用**的尾段。
不要为第二因子另写一套建立会话的逻辑——
"MFA 已启用但不知怎么被绕过了"这类 bug 就是这样写出来的。

同理，服务端 `login` 在检测到已绑定 MFA 时**不创建会话**，只发挑战令牌。
顺序反过来（先建会话再要求验证码）会给任何停在那一步的人留下一个可用会话。
