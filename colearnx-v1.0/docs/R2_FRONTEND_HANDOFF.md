...
如后端要求客户端校验和，可额外传 `sha256`；它不能替代服务端对象检查。当前规则为：PDF、DOCX、ZIP 和图片单文件最多 `26214400` bytes（25 MiB），MP4 最多 `104857600` bytes（100 MiB）；每位 Creator 总文件配额为 500 MiB，同时最多三个未完成上传。前端镜像格式和单文件大小限制用于快速反馈；总量和并发数始终由后端强制校验。
