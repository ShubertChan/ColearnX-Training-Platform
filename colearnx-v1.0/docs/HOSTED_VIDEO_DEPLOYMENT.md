...
- optional `VIDEO_WORKER_IDLE_EXIT_SECONDS`: unset/`0` keeps continuous operation; `30..86400` enables a drain-and-exit idle window. The local batch configuration uses `120` and `restart: "no"`. See [local batch setup](LOCAL_VIDEO_BATCH_WORKER.md) for scripts, tests and limitations.

For on-demand operation, start a batch after uploads are queued. An idle exit requires no locally active work and no `created`, `retry` or `active` queue jobs, including delayed retries. SIGTERM stops accepting work and waits for current transcodes, uploads, acknowledgements and cleanup before disconnecting. Use the provided Stop action and wait for `exited` before shutting down the computer. It does not wake a stopped computer or guarantee recovery from an OS-level force kill.

API queue clients are short-lived: each enqueue closes its pg-boss pool/timers in `finally`, including after a failed startup. Deploy this API change as well as the local worker; otherwise the old producer's queue-cache timer can continue waking Neon. Other website or health-check traffic may also keep the database active.
