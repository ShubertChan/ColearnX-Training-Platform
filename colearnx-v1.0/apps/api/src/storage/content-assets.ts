...
const assetSelectionInput = z.object({
...
async function discardIncompleteDraftAssets(client: PoolClient, contentVersionId: string, ownerUserId: string) {
  const result = await client.query<AssetRow>(`UPDATE storage_assets
    SET asset_status = 'delete_pending', updated_at = now()
    WHERE content_version_id = $1 AND owner_user_id = $2
      AND asset_status IN ('pending', 'uploaded', 'quarantined')
    RETURNING storage_asset_id, content_version_id, owner_user_id, bucket_name, object_key, original_filename,
      ...
      upload_expires_at`, [contentVersionId, ownerUserId]);
  return result.rows;
}
...
export async function completeUploadIntent(req: Request, res: Response) {
  ...
  const outcome = await withTransaction(async (client) => {
    ...
    // Keep the first verified asset as the backwards-compatible primary file.
    // Other verified assets remain attached to the same content version.
    await client.query(`UPDATE content_versions
      SET storage_asset_id = COALESCE(storage_asset_id, $2)
      WHERE content_version_id = $1`, [contentVersionId, asset.storage_asset_id]);
    ...
    return { kind: 'ready' as const, asset: { ...asset, asset_status: 'ready', verified_content_type: asset.declared_content_type,
      verified_byte_size: asset.declared_byte_size } };
  });
...
export async function deleteUploadIntent(req: Request, res: Response) {
  ...
  const outcome = await withTransaction(async (client) => {
    ...
    if (version.storage_asset_id === stored.storage_asset_id) {
      const replacement = await client.query<{ storage_asset_id: string }>(`SELECT storage_asset_id
        FROM storage_assets
  ...
  if (outcome.asset_status !== 'deleted') await bestEffortDelete(outcome);
...
export async function listContentAssets(req: Request, res: Response) {
  const actor = res.locals.actor as Actor;
  const contentVersionId = parse(uuid, req.params.contentVersionId);
  const access = await query<ContentVersionAccess>(`SELECT c.creator_user_id, c.publication_status, cv.version_status,
    ...
    WHERE cv.content_version_id = $1`, [contentVersionId, actor.id]);
  if (!access.rowCount) throw new ApiError(404, 'CONTENT_VERSION_NOT_FOUND', 'Content version was not found.');

  const version = access.rows[0];
  const privileged = version.creator_user_id === actor.id || actor.roles.includes('admin');
  const publicDownloadable = version.publication_status === 'published' && version.version_status === 'published';
  if (!privileged && (!publicDownloadable || !version.has_grant)) {
    throw new ApiError(403, 'CONTENT_ACCESS_DENIED', 'You do not have access to these content files.');
  }

  const assets = await query<AssetRow>(`SELECT storage_asset_id, content_version_id, owner_user_id, bucket_name,
      object_key, original_filename, declared_content_type, declared_byte_size, verified_content_type,
      verified_byte_size, etag, asset_status, upload_expires_at
    FROM storage_assets
    WHERE content_version_id = $1
      AND ${privileged ? "asset_status <> 'deleted'" : "asset_status = 'ready'"}
    ORDER BY created_at ASC`, [contentVersionId]);
...
export async function previewContentAsset(req: Request, res: Response) {
  ...
  if (!actor.roles.includes('admin')) {
    throw new ApiError(403, 'ADMIN_ROLE_REQUIRED', 'Administrator access is required to preview content files.');
  }
  const contentVersionId = parse(uuid, req.params.contentVersionId);
  const selectedAssetId = parse(assetSelectionInput, req.body ?? {}).assetId ?? null;
  const result = await query<AssetRow>(`SELECT sa.storage_asset_id, sa.content_version_id, sa.owner_user_id, sa.bucket_name,
    ...
    FROM content_versions cv
    JOIN storage_assets sa ON sa.content_version_id = cv.content_version_id
    WHERE cv.content_version_id = $1 AND sa.asset_status = 'ready'
      AND ($2::uuid IS NULL OR sa.storage_asset_id = $2)
    ORDER BY CASE WHEN sa.storage_asset_id = cv.storage_asset_id THEN 0 ELSE 1 END, sa.created_at ASC
    LIMIT 1`, [contentVersionId, selectedAssetId]);
...
export async function createContentDownloadUrl(req: Request, res: Response) {
  ...
  const result = await query<AssetRow & { creator_user_id: string; has_grant: boolean }>(`SELECT sa.storage_asset_id,
      ...
      EXISTS (SELECT 1 FROM content_access_grants cag
        WHERE cag.content_version_id = cv.content_version_id AND cag.user_id = $3
          AND (cag.expires_at IS NULL OR cag.expires_at > now())) AS has_grant
    FROM content_versions cv
    JOIN contents c ON c.content_id = cv.content_id
    JOIN storage_assets sa ON sa.content_version_id = cv.content_version_id
    WHERE cv.content_version_id = $1 AND c.publication_status = 'published'
      AND cv.version_status = 'published' AND sa.asset_status = 'ready'
      AND ($2::uuid IS NULL OR sa.storage_asset_id = $2)
    ORDER BY CASE WHEN sa.storage_asset_id = cv.storage_asset_id THEN 0 ELSE 1 END, sa.created_at ASC
    LIMIT 1`, [contentVersionId, selectedAssetId, actor.id]);
