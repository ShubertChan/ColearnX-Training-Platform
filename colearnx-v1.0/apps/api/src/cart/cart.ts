...
async function cartProduct(client: PoolClient, item: z.infer<typeof cartItemInput>) {
  if (item.kind === 'course') {
    const course = await client.query<{ id: string; seller_user_id: string; title: string; price_points: string; delivery_modes: string[] }>(`SELECT cr.course_run_id AS id,
      c.owner_user_id AS seller_user_id, c.title, cr.price_points,
      ARRAY(SELECT delivery_type FROM course_delivery_options WHERE course_run_id = cr.course_run_id AND option_status = 'active') AS delivery_modes
    ...
    return { itemType: 'course_run', ...course.rows[0], policy: purchaseRefundPolicyPreview('course', course.rows[0].delivery_modes) };
  }
  ...
  return { itemType: 'content_version', ...content.rows[0], policy: purchaseRefundPolicyPreview('content') };
}
...
export async function listCart(req: Request, res: Response) {
  ...
  const result = await query(`SELECT ci.cart_item_id, ci.item_type, ci.course_run_id, ci.content_version_id, ci.seller_user_id,
      COALESCE(ci.last_seen_price_points, ci.points_snapshot) AS last_seen_price_points, ci.policy_preview_snapshot_json, ci.added_at,
...
export async function addCartItem(req: Request, res: Response) {
  ...
  const result = await withTransaction(async (client) => {
    ...
    const stored = await client.query<{ cart_item_id: string }>(`INSERT INTO cart_items
      (cart_id, item_type, course_run_id, content_version_id, seller_user_id, points_snapshot, last_seen_price_points, policy_preview_snapshot_json)
